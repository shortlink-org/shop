...
- [ ] **Оформить корзину (Checkout)**
  - [ ] Добавить адрес доставки при оформлении заказа
  - [ ] Выбрать период доставки
  - [ ] Переход от корзины к заказу с данными доставки
  - [ ] Валидация данных доставки перед созданием заказа

**План реализации:**
1. Расширить `CreateOrderRequest` в OMS для включения:
   - `delivery_address` (адрес доставки)
   - `delivery_period` (период доставки: дата/время)
2. Обновить Order workflow для обработки данных доставки
3. Добавить валидацию адреса доставки
4. Передать данные доставки в Delivery Service при создании заказа
...
- [ ] Расширить `CreateOrderRequest` для поддержки:
  - Адреса доставки
  - Периода доставки
  - Контактной информации получателя
...
| Оформить корзину с адресом доставки | OMS Order | ❌ Требуется | Высокий |
| Оформить корзину с периодом доставки | OMS Order | ❌ Требуется | Высокий |
